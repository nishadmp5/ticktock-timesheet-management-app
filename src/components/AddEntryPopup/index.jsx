...
import useAddEntryForm from "@/customHooks/useAddEntryForm";
...
const AddEntryPopup = ({ showModal, setShowModal }) => {
  ...
  const { handleSubmit, handleReset } = useAddEntryForm(
    formApiRef,
    setShowModal
  );
  ...
  return (
    <Modal show={showModal} handleClose={() => setShowModal(false)} >
      <div className="w-full bg-white flex flex-col rounded-lg overflow-hidden">
        ...
        <Form formApiRef={formApiRef} onSubmit={handleSubmit}>
          <div className="p-5 flex flex-col gap-4">
            ...
            <div className="w-full max-w-123.5">
              <InformedTextArea
                id="task-desc"
                name="task-desc"
            ...
            <div className="w-full max-w-28.5">
              <InformedNumberInput
                id="hours"
                name="hours"
                label="Hours *"
          ...
          <div className="p-5 flex items-center justify-between gap-4 border-t border-t-[#E5E7EB]">
            <button
              type="submit"
              className="w-full rounded-lg bg-primary hover:bg-primary-dark transition-all. cursor-pointer text-14 font-medium leading-5  p-2 text-white"
            >
              Add entry
            </button>
            <button
              onClick={handleReset}
              className="w-full rounded-lg bg-white hover:bg-[#E5E7EB] transition-all cursor-pointer text-12 font-medium leading-5 border border-[#E5E7EB]  p-2 text-secondary-dark "
            >
              Cancel
            </button>
          </div>
